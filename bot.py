...
def _replace_russian_amounts(text: str) -> str:
    ...
    replacements = [
        ...
        (r"две\s+с\s+половиной\s+тыщ", "2500"),
        (r"два\s+с\s+половиной\s+тыщ", "2500"),
        (r"полторы?\s+тысяч[иеёауы]?", "1500"),
        (r"полтора?\s+тыщ", "1500"),   # полтора/полторы тыщи
        (r"полторы?\s+тыщ", "1500"),
        # три/четыре/пять/... с половиной тыщи
        (r"три\s+с\s+половиной\s+тысяч[иеёауы]?", "3500"),
        (r"три\s+с\s+половиной\s+тыщ", "3500"),
        (r"четыре\s+с\s+половиной\s+тысяч[иеёауы]?", "4500"),
        (r"четыре\s+с\s+половиной\s+тыщ", "4500"),
        (r"пять\s+с\s+половиной\s+тысяч[иеёауы]?", "5500"),
        (r"пять\s+с\s+половиной\s+тыщ", "5500"),
        (r"шесть\s+с\s+половиной\s+тысяч[иеёауы]?", "6500"),
        (r"шесть\s+с\s+половиной\s+тыщ", "6500"),
        (r"семь\s+с\s+половиной\s+тысяч[иеёауы]?", "7500"),
        (r"семь\s+с\s+половиной\s+тыщ", "7500"),
        (r"восемь\s+с\s+половиной\s+тысяч[иеёауы]?", "8500"),
        (r"восемь\s+с\s+половиной\s+тыщ", "8500"),
        (r"девять\s+с\s+половиной\s+тысяч[иеёауы]?", "9500"),
        (r"девять\s+с\s+половиной\s+тыщ", "9500"),
        (r"одна\s+с\s+половиной\s+тысяч[иеёауы]?", "1500"),
        (r"одна\s+с\s+половиной\s+тыщ", "1500"),
        # десять, одиннадцать, ... двадцать с половиной тыщи
        (r"десять\s+с\s+половиной\s+тысяч[иеёауы]?", "10500"),
        (r"десять\s+с\s+половиной\s+тыщ", "10500"),
        (r"одиннадцать\s+с\s+половиной\s+тысяч[иеёауы]?", "11500"),
        (r"одиннадцать\s+с\s+половиной\s+тыщ", "11500"),
        (r"двенадцать\s+с\s+половиной\s+тысяч[иеёауы]?", "12500"),
        (r"двенадцать\s+с\s+половиной\s+тыщ", "12500"),
        (r"тринадцать\s+с\s+половиной\s+тысяч[иеёауы]?", "13500"),
        (r"тринадцать\s+с\s+половиной\s+тыщ", "13500"),
        (r"четырнадцать\s+с\s+половиной\s+тысяч[иеёауы]?", "14500"),
        (r"четырнадцать\s+с\s+половиной\s+тыщ", "14500"),
        (r"пятнадцать\s+с\s+половиной\s+тысяч[иеёауы]?", "15500"),
        (r"пятнадцать\s+с\s+половиной\s+тыщ", "15500"),
        (r"двадцать\s+с\s+половиной\s+тысяч[иеёауы]?", "20500"),
        (r"двадцать\s+с\s+половиной\s+тыщ", "20500"),
        # голос: «2 с половиной тыщи» / «3,5 тысячи» (любая цифра уже выше)
        (r"(\d+)\s+с\s+половиной\s+тысяч[иеёауы]?", lambda m: str(int(m.group(1)) * 1000 + 500)),
        (r"(\d+)\s+с\s+половиной\s+тыщ", lambda m: str(int(m.group(1)) * 1000 + 500)),
        (r"(\d+)\s*[,.]?\s*5\s+тысяч[иеёауы]?", lambda m: str(int(m.group(1)) * 1000 + 500)),
        (r"(\d+)\s*[,.]?\s*5\s+тыщ", lambda m: str(int(m.group(1)) * 1000 + 500)),
        # голос часто даёт «2 тыщи» вместо «полторы тыщи» — целое число + тыщ/тыщи
        (r"(\d+)\s+тыщ[иеёауы]?", lambda m: str(int(m.group(1)) * 1000)),
        (r"две\s+тысячи", "2000"),
        (r"два\s+тысячи", "2000"),
        (r"пять\s+тысяч", "5000"),
        (r"три\s+тысячи", "3000"),
